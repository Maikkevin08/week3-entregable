...
const setGenresToMovies=catchError(async(req,res)=>{
    //movies/:id/genres
    //desestructurando el id de los parametros
    const {id}=req.params
    //busqueda de la pelicula
    const movie=await Movie.findByPk(id)
//en caso no encontrarla retornar 404
    if(!movie) return res.status(404).json({error:"Movie not found"})

    //caso encontrarla cambiar es decir setearla
    await movie.setGenres(req.body)
    //estos generos los lee y alamcena en genres
